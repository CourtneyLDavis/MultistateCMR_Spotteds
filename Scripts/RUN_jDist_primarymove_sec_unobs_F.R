...
# read in script file with model functions
source('Scripts/FUNCTIONS_jDist_primarymove_sec_unobs_F.R')

# read in data for primary level (between-year observations)
Z <- read.table('Data/attendance_12ponds_primarymove_females.txt', header=F)

# read in data for secondary level (within-year observations)
X <- NULL
X[[1]] <- read.table('Data/state2014_females.txt', header=F)
X[[2]] <- read.table('Data/state2015_females.txt', header=F)
X[[3]] <- read.table('Data/state2016_females.txt', header=F)
X[[4]] <- read.table('Data/state2017_females.txt', header=F)
X[[5]] <- read.table('Data/state2018_females.txt', header=F)
X[[6]] <- read.table('Data/state2019_females.txt', header=F)
...
## Run to calculate bootstrap 95% confidence intervals
source('BOOTSTRAPS_jDist_primarymove_sec_unobs_F.R')
